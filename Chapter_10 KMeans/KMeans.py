...
def kmeans(data, k, centroids):
    ...
    subCenter = np.mat(np.zeros((m, 2)))  # 初始化每一个样本所属的类别      （第一列：minIndex  第二列：minDist）
    ...
    while change == True:
        ...
        for i in range(m):
            ...
            if subCenter[i, 0] != minIndex:  # 需要改变
                change = True
                subCenter[i, ] = np.mat([minIndex, minDist])  #改变样本所属的类别
        # 重新计算聚类中心
        for j in range(k):
            ...
            for z in range(n): 
                try:
                    centroids[j, z] = sum_all[0, z] / r   #将同一个类别的样本数据相加 求平均值得到该类别聚类中心 
                except:
                    print (" r is zero")
    return subCenter
